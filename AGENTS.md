...
- The bottom-right telemetry widget is a single consolidated box containing the Traffic Monitor toggle, the spike-trigger, the active TARGET telemetry, and (in geographic projection) the GEO LOCKED name/address/sector row and the map zoom controls (−/＋/reset). Do not reintroduce separate top-left traffic monitor, bottom-left location badges, or a bottom-right geomatic zoom panel.
- The geographic overlay keeps a right-edge HUD chrome stack (compass ring, cursor range/bearing readout, nominal scale bar), a target-lock bracket reticle centered on the GEO LOCKED node, and low-opacity orbital satellite crossers. The geo zoom offset is lifted into OperationSphere's telemetry box.
- The North America (NA) continental view renders `public/maps/us-node-graph-underlay.webp` (sourced from `assets/`) as a style-aware base map (0.38 on Tactical Vector, 0.5 on Satellite Recon). Keep it gated to the `na` node so EU/AS views keep their vector landmass, and keep the underlay below the grid lines and pins.